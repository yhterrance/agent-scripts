...
Sign-in needed (e.g. Clerk) => reuse Chrome login state. One profile on this machine: `Default` (Terrance) — use it for `localhost:3000` Tracing Paper, etc.

```bash
agent-browser --headed --profile Default --enable react-devtools open http://localhost:3000/...
```

`--profile` snapshots the profile to a temp dir (read-only). No risk to original Chrome state — but the snapshot **copies the profile's cookies/sessions**, so a logged-in account rides along. For e2e flows that need a different identity, skip `--profile` and use a clean isolated session (`AGENT_BROWSER_SESSION_NAME=...`), signing in via the API.

`--enable react-devtools` is default-on for the `Default` profile because it's used for React apps and the flag only applies at launch — forgetting it means `close --all` + relaunch + reauth. Drop it only when you want zero React instrumentation overhead (e.g. micro-perf measurement). Same for init-scripts and other `--enable` flags: launch-time only; CDP-attaching (`--cdp 9222`) skips them.
